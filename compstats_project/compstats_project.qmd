...
3. Sample $\mathcal{B}$ and $\mathcal{C}$ from their conditiopnal distribution $p(\mathcal{B},\mathcal{C}\mid\theta^t,r,u,\varepsilon)$; where $\mathcal{B}$ collects every state visited by leapfrog integration during an iteration, and $\mathcal{C}\subseteq \mathcal{B}$ are the states eligible as the next state.  
...
* however after 20,000 iterations, even though NUTS obtains a sample with less autocorrelation, quality of approximation similar to MH
...
:::{.centering}
![](figures/three_peak_trace_1000.svg){width="100%" .fragment}
:::
...
:::{.centering}
![](figures/three_peak_acf.svg){width="100%" .fragment}
:::

## Additional Slides

Slide content to be included in the final version of the slide set.
